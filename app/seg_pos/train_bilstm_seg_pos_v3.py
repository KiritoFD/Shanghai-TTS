"""BiLSTM-CRF joint segmentation + POS tagging v3.

Key changes from v2:
  - Smaller model (256/2-layer) better suited for 50k data
  - Capped POS class weights to avoid extreme values
  - Label smoothing for POS classification
  - Reduced retrieval weight (it distracts from seq tasks)
  - LayerNorm + separate embedding dropout
  - Linear warmup then cosine decay
  - More frequent eval + early stopping
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output_dir", default="app/seg_pos/outputs/bilstm_seg_pos_v3", type=str)
    ...
    parser.add_argument("--seg_loss_weight", default=2.0, type=float)
    parser.add_argument("--pos_loss_weight", default=1.0, type=float)
    parser.add_argument("--retrieval_loss_weight", default=0.05, type=float)
    parser.add_argument("--pos_weight_max", default=5.0, type=float)
    parser.add_argument("--label_smoothing", default=0.1, type=float)
    parser.add_argument("--use_crf", default=True, type=bool)
    parser.add_argument("--max_length", default=96, type=int)
    parser.add_argument("--eval_every_steps", default=400, type=int)
...
def compute_pos_weights(train_rows: list[dict], num_pos_tags: int, max_weight: float = 5.0) -> torch.Tensor:
    ...
    for row in train_rows:
        for tag in row.get("pos_tags", []):
            ...
            if 0 <= tid < num_pos_tags:
                counts[tid] += 1
    total = sum(counts)
    raw = [total / (num_pos_tags * c) for c in counts]
    mx = max(raw)
    capped = [min(w / mx, max_weight) / max_weight for w in raw]
...
class BiLSTMCRFSegPos(nn.Module):
    ...
    def contextualize(self, ids: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        embedded = self.emb_dropout(self.embedding(ids))
...
def train_loss(
    ...
    pos_labels_clamped = batch["pos_labels"].clamp(min=0)
    pos_valid = batch["pos_labels"] != -100
    if pos_valid.any():
        pos_loss = F.cross_entropy(
            pos_logits.reshape(-1, pos_logits.size(-1)),
            pos_labels_clamped.reshape(-1),
            ignore_index=-100,
...
def main() -> None:
    ...
    pos_weights = compute_pos_weights(train_rows, len(POS_TAGS), args.pos_weight_max)
